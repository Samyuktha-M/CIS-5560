...
#paramGrid = ParamGridBuilder().addGrid(lr.maxIter, [10, 5]).build()  

#paramGrid = (ParamGridBuilder()
#             .addGrid(lr.maxDepth, [2, 5, 10])
#             .addGrid(lr.maxBins, [10, 20, 40])
#             .addGrid(lr.maxIter, [5, 10, 20])
#             .build())
paramGrid = (ParamGridBuilder()\
            .addGrid(gbt.maxDepth,[2,3,29])\
            .addGrid(gbt.minInfoGain,[0.0, 0.7])\
	    .addGrid(gbt.maxBins,[40,100])\
            .addGrid(gbt.stepSize,[0.05, 0.2])\
            .addGrid(gbt.maxIter, [2, 10])\
            .build())
val = CrossValidator(estimator=pipeline, evaluator= BinaryClassificationEvaluator(), estimatorParamMaps=paramGrid,numFolds=5)
