...
data =csv.select('Host Listings Count','Host Total Listings Count',"Neighbourhood","Property Type","Room Type","Bed Type","Latitude","Longitude","Accommodates","Bathrooms","Bedrooms","Monthly Price","Guests Included","Extra People","Minimum Nights","Review Scores Rating","Review Scores Accuracy","Review Scores Cleanliness","Review Scores Checkin","Review Scores Communication","Review Scores Location","Review Scores Value","Sentiment",col("Price").cast("Int").alias("label"))
...
outliers = data.stat.approxQuantile(['label','Host Listings Count','Host Total Listings Count',"Accommodates","Bathrooms","Bedrooms","Monthly Price","Guests Included","Extra People","Minimum Nights"],[0.05,0.95],0.0)
...
data = data.filter((f.col('Guests Included') >= outliers[7][0]) & (f.col('Guests Included') <= outliers[7][1]))
data = data.filter((f.col('Extra People') >= outliers[8][0]) & (f.col('Extra People') <= outliers[8][1]))
data = data.filter((f.col('Minimum Nights') >= outliers[9][0]) & (f.col('Minimum Nights') <= outliers[9][1]))
...
data_clean = StringIndexer(inputCol='Property Type', outputCol='PropertyType_index').setHandleInvalid("keep").fit(data_clean).transform(data_clean)
data_clean= StringIndexer(inputCol='Room Type', outputCol='RoomType_index').setHandleInvalid("keep").fit(data_clean).transform(data_clean)
...
catVect = VectorAssembler(inputCols =['PropertyType_index', 'RoomType_index'], outputCol="catFeatures")
...
gbt = GBTRegressor(labelCol="label", featuresCol="features",maxBins=40)
